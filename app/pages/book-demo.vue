...
useSeoMeta({
  title: () => t("book_demo.meta.title"),
  description: () => t("book_demo.meta.description"),
  ogTitle: () => t("book_demo.meta.title"),
  ogDescription: () => t("book_demo.meta.description"),
...
const features = computed(() => [
  {
    ...
    text: t("book_demo.features.sms"),
  },
  {
    icon: "i-heroicons-paper-airplane",
    text: t("book_demo.features.campaigns"),
  },
  {
    icon: "i-heroicons-users",
    text: t("book_demo.features.channels"),
  },
  {
    icon: "i-heroicons-chart-bar",
    text: t("book_demo.features.analytics"),
  },
]);
</script>
      ...
      <div class="grid lg:grid-cols-2 gap-12 lg:gap-24 items-center">
        ...
        <div class="space-y-8">
          ...
          >
            ...
            <span class="text-sm">{{ $t("book_demo.meta_partner") }}</span>
          </div>
        ...
        >
          <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-8">
            {{ $t("book_demo.form.title") }}
          </h2>

          <form class="space-y-6" @submit.prevent="onSubmit">
            <!-- Name Field -->
            <UFormField
              :label="$t('book_demo.form.name')"
              name="name"
              class="w-full"
              :ui="{
                label: {
            ...
            >
              <template #label>
                <div class="flex items-center gap-2">
                  ...
                  />
                  <span>{{ $t("book_demo.form.name") }}</span>
                </div>
              </template>
              <UInput
                v-model="form.name"
              ...
              <template #label>
                <span class="flex items-center gap-2">
                  <UIcon
                    ...
                    class="w-4 h-4 text-blue-500 dark:text-blue-400"
                  />
                  <span>{{ $t("book_demo.form.phone") }}</span>
                </span>
              </template>
              <UInput
                v-model="form.phone"
              ...
              <template #label>
                <div class="flex items-center gap-2">
                  <UIcon
                    ...
                    class="w-4 h-4 text-blue-500 dark:text-blue-400"
                  />
                  <span>{{ $t("book_demo.form.email") }}</span>
                </div>
              </template>
              <UInput
                v-model="form.email"
            ...
            <!-- Company Name Field -->
            <UFormField
              :label="$t('book_demo.form.company')"
              name="company"
              :ui="{
                label: {
                  base: 'text-gray-700 dark:text-gray-300 font-medium mb-2',
              ...
              <template #label>
                <div class="flex items-center gap-2">
                  <UIcon
                    ...
                    class="w-4 h-4 text-blue-500 dark:text-blue-400"
                  />
                  <span>{{ $t("book_demo.form.company") }}</span>
                </div>
              </template>
              <UInput
            ...
            <!-- Submit Button -->
            <UButton type="submit" block size="xl" color="primary">
              {{ $t("book_demo.form.submit") }}
            </UButton>
          </form>
        </div>
      </div>
